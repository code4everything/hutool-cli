...
笔者也是一个Hutool深度使用者，很多方法可以说都是一个小工具，比如BASE64的编码与解码、计算器、时间戳与日期的转换、随机字符串、二维码的生成与解码，正则表达式测试等等，而且都是调用一个静态方法即可完成。
...
> 如果你熟悉Hutool，那么使用本工具毫无困难，几乎没有学习成本；如果你不熟悉Hutool，那么本工具可以带你走进Hutool；但是如果你不熟悉Java，那使用本工具可能稍显困难。

## 终端命令参数解析

要实现一个终端命令，首先我们需要一个命令参数解析库，搜了一圈，发现Java的命令解析库还是挺多的，常用的有JCommander、CommonsCli、Args4J等，最后综合对比下来，选择了JCommander，文档结构十分清晰，使用起来也非常简单顺手。

支持数组解析，我们可以将多个参数解析到同一个数组中，比如：`--arr 1 --arr 2 --arr 3` 解析到 `@Parameter(names = "--arr") List<Integer> arr` 中，这特性可以应用到本工具执行静态方法时所需参数的传入功能。

> [JCommander传送门](https://jcommander.org)

## 执行静态方法

有了终端命令参数解析的支持，那我们就可以设计参数，命令需要传入类全名、方法名、参数类型、方法参数值，通过传入的信息去匹配静态方法，并通过反射去调用它，最后将结果输出到终端。

用于方法定位的参数设计如下，其中为了简化输入，方法采用了模糊匹配，忽略方法名的大小写敏感，方法的参数类型也设计为非必填字段，为空时会根据具体的方法入参个数进行匹配调用。
...
将字符串 `123456789` 进行BASE64编码，调用方法 `cn.hutool.core.codec.Base64#encode('123456789')`，命令示例：`-c cn.hutool.core.codec.Base64 -m encode -t java.lang.CharSequence -p 123456789`。

> 当然如果缺省方法参数类型时，可能导致方法匹配错误，进而方法参数转换失败，最终导致方法调用失败。
...
文件转换器的实现，从代码中可以看出传入的是文件路径，并且支持相对路径，命令示例：`-c cn.hutool.core.io.FileUtil -m readUtf8String -t java.io.File -p test.txt`。
...
日期转换器的实现，同样从代码可以看出，日期除了支持常见的格式，比如：yyyy-MM-dd, yyyy-MM-dd HH:mm:ss 等，还支持三个字符串：now, yesterday, tomorrow。
...
目前引入的转换器也不多，毕竟用到的方法有限，转换器的引入也十分简单，支持动态引入，需要时可随时添加。

> Java常用类型自动转换，无需引入方法参数转换器，比如：Boolean, Integer, Long, Float, Double等等，这些类型会使用fastjson的TypeUtils#cast方法转换。

## 别名设计

上面的示例命令你也看见了，如果一个功能需要如此冗长的命令，而且还需要你谨记类名和方法名才能得以执行，相信是不会有人使用的。
...
别名的参数解析设计如下，我们可以通过 `-r` 来执行一个别名，并且它还可以省略，`-r alias` 和 `alias` 效果是一样的，别名后面还可以直接跟参数，无需 `-p`。
...
到这里，我们就可以通过一个很简单的命令去执行一个方法了，比如：`pinyin 魑魅魍魉` 就可以输出魑魅魍魉的拼音。

> [内置类别名表](README.md#常用类名简写对照表)

### 输出方法形参名称

截止到这里，hutool-cli已经可以很方便的执行一个方法了，我们一个名叫 `methods` 的别名来查看一个类所有的静态方法，但是输出少了一个灵魂，那就是方法形参的名称，你可以想象如果一个方法有三五个参数，我们现在只能看到参数类型，不知道参数名称，不看Hutool源代码根本不知道每个参数是用来干嘛的。

为了能够输出方法的形参名称，引入 `javassist` 字节码库，这部分也挺简单的，输出格式：methodName(paramName:paramType)。
...
到这里我们还只能通过 `java -jar hutool.jar -r alias` 命令来执行，还需要一段能打包成二进制可执行文件的代码，综合下来选择了`go`语言来完成这项工作。

现在我们可以这样来执行`hutool alias`，至此hutool-cli基本功能及其实现已全部介绍完成。

> [源代码查看](src/main/go/hutool.go)

### 总结

本工具实现倒是挺简单的，技术也不复杂，涉及到的类也不多，除了Hutool库，主要引入了JCommander和Javassist库。但是别名定义的工作量还是蛮大的，这是主要耗时的地方。
